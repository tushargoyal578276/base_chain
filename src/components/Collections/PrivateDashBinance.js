...
import { deposit_busd, busd, feed } from '../../abi/address'
    ...
    const providerOptions = {
        walletconnect: {
            ...
            options: {
                network: 'bsc',
                rpc: {
                    56: "https://bsc-dataseed4.defibit.io"
                },
                chainId: 56
            }
        },
...
function PrivateDashBinance() {
    const [acc, setacc] = useState()
    const [accountid, setaccountid] = useState();
    const [web3, setWeb3] = useState();
    const [netId, setNetId] = useState();
    const [busdDeposit, setBusdDeposit] = useState();
    const [depositAmt, setDepositAmt] = useState(0);
    const [depositQty, setDepositQty] = useState(0);
    const [enableTokenTxt, setEnableTokenTxt] = useState('Enable BUSD');
    const [buyTokenTxt, setBuyTokenTxt] = useState('Buy tokens');
    const [mybalance, setMybalance] = useState();
    const [busdCont, setBusdCont] = useState();
    ...
    useEffect(async () => {
        ...
        if (acc) {
            ...
            let _busdDeposit = new web3_2.eth.Contract(contractJson, deposit_busd);

            let busdCont = new web3_2.eth.Contract(Erc20, busd);
            setBusdCont(busdCont);

            setBusdDeposit(_busdDeposit);
            setProviderEvent();

            const allowance = await busdCont.methods.allowance(accounts[0], deposit_busd).call({ from: accounts[0] });
    ...
    async function enableDeposit() {
        ...
        if (tokenType == 'BUSD') {
            try {
                ...
                await busdCont.methods.approve(deposit_busd, toHex).send({ from: accountid});
                NotificationManager.success('Approved BUSD', 'PriceAI');
            } catch (error) {
                NotificationManager.error('Get errors in apporve BUSD', 'PriceAI');
                setEnableTokenTxt('Enable ' + tokenType);
                return false;
            }                        
    
            const allowance = await busdCont.methods.allowance(accountid, deposit_busd).call({ from: accountid });
    ...
    async function getMyTokenAmt(_tokenName) {
        ...
        if (_tokenName == 'BUSD') {
            const total_ = await busdCont.methods.balanceOf(accountid).call();
            let weiTotal =  web3.utils.fromWei(total_);
    ...
    async function getMyBalance(_tokenName) {
        ...
        let depositResultBUSD = 0;
        depositResultBUSD = await busdDeposit.methods.getDepositedQty(accountid).call().then((result) => {
            return result;
        })           


        setMybalance(Number(web3.utils.fromWei(depositResultBUSD)));
    }
    ...
    async function deposit(_tokenName) {
        ...
        try {
            const value = fromDecimals({ amount:depositAmt, decimals: 18 })
            const qty = fromDecimals({ amount:depositQty, decimals: 18 })            

            let depositResult = false;

            if (_tokenName == 'BUSD') {
                depositResult = await busdDeposit.methods.depositToken(qty, value, 2, true).send({ from: accountid }).then((result) => {
                    ...
                    <div className="container">
                            ...
                            <div className="col-md-3 col-lg-2">
                                <div className="tokens-left-item">
                                    ...
                                    <div className='neticon-wrapper'>
                                        <a href='/sale'><img src="assets/img/sale/icon1_fade.svg" alt=""/></a>
                                        <a href='#'><img src="assets/img/sale/icon3.svg" alt=""/></a>
                                    </div>
                        ...
                        <div className="overview-main">
                            ...
                            <ul>
                                ...
                                <li><span><img src="assets/img/sale/ovr3.svg" alt=""/></span> $0.001 BUSD</li>
...
export default PrivateDashBinance;